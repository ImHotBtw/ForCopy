...
document.querySelector("#toggleHud").setAttribute("onclick", "showHud()");
  ...
  document.querySelector(".social-container").innerHTML += '<a id="toggleHud" style="background:#c00;cursor:pointer;outline:none;border:0;padding:5px;color:#dadada;box-shadow:0 0 1px 1px #000;border-radius:4px;font-size:16px;text-shadow:1px 1px 2px #000;margin-left:10px;" onclick="showHud()">Login Panel</a>';
	...
	function flixPanel() {
		...
		.flix > .flix-skins > .flix-skins-item {
		
		  width: 100%;
		}
		.flix > .flix-skins > .flix-skins-item > p {
		...
		.flix > .flix-skins > .flix-skins-item > input {
		...
		.flix > .flix-extras > .flix-extras-item {
		
		  width: 100%;
		  display: inline-flex;
		}
		.flix > .flix-extras > .flix-extras-item > p {
		...
		.flix > .flix-extras > .flix-extras-item > input {
		...
		<div class="flix">
		  <div class="flix-hud">
		<h1>Flix Panel</h1>
			<p>Leaderboard Color</p>
			<input id="flix-hc" type="text" placeholder="hex/rgb ex. hex - #ffffff | rgb - (255, 255, 255)">
		  </div>
		  ...
		  <div class="flix-extras">
			...
			<div class="flix-extras-item">
			  <p>Skip Stats</p>
			  <input id="flix-e-ss" type="checkbox">
			</div>
		...
		setTimeout(()=>{
		window.showHud = () => {
		document.querySelector(".flix").style.display ="block";
		document.querySelector("#toggleHud").setAttribute("onclick", "hideHud()");
		
		}
		
		window.hideHud = () => {
		document.querySelector(".flix").style.display ="none";
		document.querySelector("#toggleHud").setAttribute("onclick", "showHud()");
		}
		
		  document.querySelectorAll("#vanis-io_300x250")[0].innerHTML += flix;
		...
		let flix_ROTATOR_STATE = false;
		const flix_ROTATOR_DELAY = 1000;
		
		
		let flix_EXTRAS_AR = false;
		let flix_EXTRAS_AR_INTERVAL = null;
		
		let flix_EXTRAS_SS = false;
		let flix_EXTRAS_SS_INTERVAL = null;
		
		const flix_DOM_OVERLAY = _$(".flix");
		
		const flix_DOM_HC = _$("#flix-hc");
		
		const flix_DOM_SKIN_INPUT = _$("#skinurl");
		const flix_DOM_SKIN1 = _$("#flix-s1");
		const flix_DOM_SKIN2 = _$("#flix-s2");
		const flix_DOM_SKIN3 = _$("#flix-s3");
		const flix_DOM_SKIN4 = _$("#flix-s4");
		
		const flix_DOM_ROTATOR_START = _$("#flix-r-start");
		const flix_DOM_ROTATOR_STOP = _$("#flix-r-stop");
		
		const flix_DOM_EXTRAS_AR = _$("#flix-e-ar");
		const flix_DOM_EXTRAS_SS = _$("#flix-e-ss");
		
		const flix_HUD_COLOR_FUNC = () => {
		
		  for(const element of _$(".fade"))
			element.style.background = flix_HUD_COLOR;
		
		  _$("#overlay").style.background = "radial-gradient("+flix_HUD_COLOR+" 300px,"+flix_HUD_COLOR+")";
		};
		
		let flix_ROTATOR_INTERVAL = null;
		let flix_ROTATOR_CURRENT = 1;
		
		const flix_ROTATOR_FUNC_NEXT = () => {
		
		  if(flix_ROTATOR_CURRENT < 5)
			flix_ROTATOR_CURRENT += 1;
		
		  if(flix_ROTATOR_CURRENT == 5)
			flix_ROTATOR_CURRENT = 1;
		
		  flix_DOM_SKIN_INPUT.value = localStorage.getItem("flix-s"+flix_ROTATOR_CURRENT);
		};
		
		const flix_ROTATOR_FUNC_PREV = () => {
		
		  if(flix_ROTATOR_CURRENT < 5)
			flix_ROTATOR_CURRENT -= 1;
		
		  if(flix_ROTATOR_CURRENT == 0)
			flix_ROTATOR_CURRENT = 1;
		...
		flix_DOM_HC.addEventListener("change", event => {
		
		  if(event.target.value == localStorage.getItem("flix-hc"))
			return;
		
		  localStorage.setItem("flix-hc", event.target.value);
		
		  flix_HUD_COLOR = localStorage.getItem("flix-hc");
		
		  flix_HUD_COLOR_FUNC();
		}, false)
		
		flix_DOM_SKIN1.addEventListener("change", event => {
		
		  if(event.target.value == localStorage.getItem("flix-s1"))
			return;
		
		  localStorage.setItem("flix-s1", event.target.value);
		
		  flix_SKIN1 = localStorage.getItem("flix-s1");
		}, false)
		
		flix_DOM_SKIN2.addEventListener("change", event => {
		
		  if(event.target.value == localStorage.getItem("flix-s1"))
			return;
		
		  localStorage.setItem("flix-s2", event.target.value);
		
		  flix_SKIN2 = localStorage.getItem("flix-s2");
		}, false)
		
		flix_DOM_SKIN3.addEventListener("change", event => {
		
		  if(event.target.value == localStorage.getItem("flix-s1"))
			return;
		
		  localStorage.setItem("flix-s3", event.target.value);
		
		  flix_SKIN3 = localStorage.getItem("flix-s3");
		}, false)
		
		flix_DOM_SKIN4.addEventListener("change", event => {
		
		  if(event.target.value == localStorage.getItem("flix-s1"))
			return;
		
		  localStorage.setItem("flix-s4", event.target.value);
		
		  flix_SKIN4 = localStorage.getItem("flix-s4");
		}, false)
		
		flix_DOM_ROTATOR_START.addEventListener('click', event => {
		
		  flix_ROTATOR_STATE = true;
		
		  flix_DOM_ROTATOR_STOP.className = "";
		  event.target.className += " active";
		
		  flix_ROTATOR_CURRENT = 1;
		  flix_ROTATOR_INTERVAL = setInterval(flix_ROTATOR_FUNC_NEXT, flix_ROTATOR_DELAY);
		}, false);
		
		flix_DOM_ROTATOR_STOP.addEventListener('click', event => {
		
		  flix_ROTATOR_STATE = false;
		
		  flix_DOM_ROTATOR_START.className = "";
		  event.target.className += " active";
		
		  flix_ROTATOR_CURRENT = 1;
		  clearInterval(flix_ROTATOR_INTERVAL);
		}, false);
		...
		flix_DOM_EXTRAS_AR.addEventListener('change', event => {
		  ...
		  clearInterval(flix_EXTRAS_AR_INTERVAL);
		  flix_EXTRAS_AR_INTERVAL = null;
		}, false);
		
		flix_DOM_EXTRAS_SS.addEventListener('change', event => {
		
		  const state = event.target.checked;
		
		  if(state == true){
		
			flix_EXTRAS_SS_INTERVAL = setInterval(()=>{
		  ...
		  clearInterval(flix_EXTRAS_SS_INTERVAL);
		  flix_EXTRAS_SS_INTERVAL = null;
		}, false);
		
		const init = () => {
		
		  flix_DOM_HC.value = localStorage.getItem("flix-hc") || "";
		  flix_HUD_COLOR = flix_DOM_HC.value;
		  flix_HUD_COLOR_FUNC();
		
		  flix_DOM_SKIN1.value = localStorage.getItem("flix-s1") || "";
		  flix_DOM_SKIN2.value = localStorage.getItem("flix-s2") || "";
		  flix_DOM_SKIN3.value = localStorage.getItem("flix-s3") || "";
		  flix_DOM_SKIN4.value = localStorage.getItem("flix-s4") || "";
		
		  flix_DOM_ROTATOR_STOP.click();
		
		  flix_DOM_EXTRAS_AR.checked = localStorage.getItem("flix-e-ar") || false;
		  flix_DOM_EXTRAS_SS.checked = localStorage.getItem("flix-e-ss") || false;
		};
